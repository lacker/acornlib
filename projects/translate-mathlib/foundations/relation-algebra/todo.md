...
- Basic relation lattice algebra is now in `src/relation_basic.ac`: relation union, equality by mutual inclusion, commutativity/associativity/idempotence, monotonicity, absorption, distributivity, converse-over-union, union preservation for reflexivity, irreflexivity, symmetry, and totality, and composition distributivity over union (`relation_compose_union_left`, `relation_compose_union_right`) with one-sided intersection inclusions (`relation_compose_intersection_left_subset`, `relation_compose_intersection_right_subset`).
